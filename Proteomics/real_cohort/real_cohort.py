...
def main():
    ...
    ap.add_argument(
        "--results_csv",
        default=None,
        help="If set, write per-fold scores, summary, and permutation-test "
             "results to CSV files derived from this path stem. This makes "
             "the figure of record reproducible from the pipeline output.",
    )
    ap.add_argument(
        "--hgnc",
        default=None,
    ...
    print(f"Permutation null mean ± sd: {perm_aucs.mean():.3f} ± {perm_aucs.std():.3f}")

    # ----------------------------------------------------------------
    # Persist all computed quantities to CSV so the figure of record
    # is reproducible from the pipeline (no hard-coded values).
    # ----------------------------------------------------------------
    if args.results_csv:
        stem = Path(args.results_csv).with_suffix("")
        op = args.operator

        # per-fold scores (the raw distribution behind the boxplot)
        pd.DataFrame({
            "operator": op,
            "fold": np.arange(len(aucs)),
            "auroc": aucs,
            "accuracy": accs,
        }).to_csv(f"{stem}_{op}_folds.csv", index=False)
        ...
        pd.DataFrame([{
            ...
            "n_metabolites_mapped": int(len(wb_mets)),
            ...
            "permtest_observed_auc": float(observed_auc),
            "permtest_null_mean": float(perm_aucs.mean()),
            "permtest_null_std": float(perm_aucs.std()),
            "permtest_p_value": float(perm_p),
            "seed": int(args.seed),
        }]).to_csv(f"{stem}_{op}_summary.csv", index=False)

        # full permutation null distribution
        pd.DataFrame({
            "operator": op,
            "perm_index": np.arange(len(perm_aucs)),
            "perm_auc": perm_aucs,
        }).to_csv(f"{stem}_{op}_permnull.csv", index=False)

        print(f"Wrote results CSVs: {stem}_{op}_folds.csv, "
              f"{stem}_{op}_summary.csv, {stem}_{op}_permnull.csv")
