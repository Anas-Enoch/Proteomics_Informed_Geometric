...
This is a **real, bimodal result** (reported from the corrected pipeline, `K_EIGS=50`).

Across **57/57** cell lines, two complementary metrics are computed:

- **eigenvalue-magnitude deviation** (`spectral_rel_l2_to_topology`): median **7.32**, range **0.003–7.37**
- **eigenvector subspace distance** (`subspace_distance_to_topology`): median **0.000**, mean **0.140**, max **0.957**
- cell lines with low-frequency geometry rotation > 0.1: **10 / 57**

Two populations, split by `n_eigs_nontrivial`:

| group | cell lines | eigenvalue L2 | eigenvector subspace dist |
|-------|-----------|---------------|---------------------------|
| stable eigenvectors (`n_eigs ≥ 15`) | 42 | median 7.33 | median ~3e-8 (≈ identical) |
| rotating eigenvectors (`n_eigs < 15`) | 15 | median 1.00 | median 0.735 (genuinely rotate) |

### Interpretation

> Proteomics-informed reaction weighting changes eigenvalue **magnitudes** broadly across essentially all cell lines, but rotates the low-frequency **eigenvector geometry** (the part that determines the spectral embedding) only for a minority of cell lines.

This bimodal pattern is internally consistent and unifying: it explains both the near-null operator-type effect under random masking (Experiment B) and the exact topology-vs-permuted tie in the real cohort, since all three analyses are sensitive to eigenvector geometry, which proteomics leaves unchanged for most samples at full model scale.

> Note: an earlier draft reported "median relative deviation 0.564"; that value came from a superseded run (`K_EIGS=5`) and is replaced by the corrected bimodal result above.
